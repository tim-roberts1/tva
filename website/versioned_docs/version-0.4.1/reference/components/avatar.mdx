...
- `options`: A String value of a theme sentiment `'default' | 'action'` which determines the styling color theme produced. A label property that accepts a String value which creates the initial fallback initials and Accessibility settings. A src property that accepts a String value for displaying an image source. An optional size property that accepts a String value of either `'xs' | 's' | 'm' | 'l' | 'xl'` to display the size styles.
