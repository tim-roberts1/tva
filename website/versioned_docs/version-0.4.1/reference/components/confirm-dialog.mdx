...
- `options`: id, headerId, and bodyId properties that accept a String value to connect and setup Accessibility properties for all the elements used within the Confirm Dialog. An optional kind property that accepts a String value of `'non-destructive' | 'destructive'` to determine the styling of the Confirm Dialog.
