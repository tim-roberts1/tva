...
An Object containing an `expanded` property of a Boolean value related to the state of the menu. A `menu` property containing both event delegation and accessbility properties to spread on the `menu` element. A `trigger` property containing event delegation, ref, and dataset properties to spread on the element responsible for triggering the state of the menu (usually a `button`).
