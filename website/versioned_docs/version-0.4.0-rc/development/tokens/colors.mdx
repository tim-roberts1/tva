...
These are the tokens we provide (i.e. a theme). Our [normalize setup](../getting-started/installation.md#normalizecss) comes with three themes: **light**, **dark**, and **system**. For every theme, we overwrite the values shown in this list so the token names are the same for all themes.
...
The design-tokens package comes with three themes: **dark (default)**, **light**, and **system**.

This website uses the "dark" and "light" themes.

Themes are controlled by using an attribute on the `html` tag of `data-theme` for a full page takeover (what we do on this site), or a CSS class of the `pando-<theme name>` (i.e. "light", "dark") for component level control.

If no theme is set, the default token values that you will receive will be for the dark theme.
