...
An Object matching the [Anatomy](#anatomy) containing `className` and a11y properties relevant to each element/section. Additionally, it returns a `closeButtonOPtions` Object with to use as the parameters for the Icon Button API.

## Behavoir

:::note Popover vs. Tooltip

A Popover should be used when you need to provide more context to a user whereas a Tooltip displays a short-description of only a few words (like the browser `title`).

:::

### Patterns

**Do** use the Popover to display non-essential information to help users that are not familair with the interface.
