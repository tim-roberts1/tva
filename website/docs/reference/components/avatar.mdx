...
function ActionableAvatar() {
  ...
  const { button } = getButtonProps({
    usage: 'text',
    classNames: [className, 'pando-avatar-btn-override'],
  })
  ...
  return (
    <div style={{ paddingTop: '1rem', paddingBottom: '1rem' }}>
      <div {...getGridProps({ cols: 12, gap: 16 })}>
        ...
        <div {...gridItemStyles}>
          <button {...button} onClick={handleAvatarClick}>
...
Each component layer of the Admontion has a unused class name that can be utilized in your local CSS to customize the Avatar at any level.
