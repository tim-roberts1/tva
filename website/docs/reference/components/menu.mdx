...
When you need to create a Submenu, just nest a new `menu` in the position you need and combine it with the [useSubmenuInteraction hook](../react-hooks/useSubmenuInteraction.mdx) to make it accessible.
...
An Object matching the [Anatomy](#anatomy) containing `className` and a11y properties relevant to the label element. Additionally, it returns a `iconOptions` property to be passed into the Icon API if needed.
...
This API does not accept any options.
