...
- `options`: A string value for properties htmlFor and value that produce style and a11y properties for the Message components.

### Returns

An Object matching the [Anatomy](#Anatomy) containing `className` and a11y properties relevant to the label element. Additionally, it returns a `container` property that adds a11y properties for a wrapper component to the message element.
...
**Do** use the Error Message on all form field's that are invalid.
