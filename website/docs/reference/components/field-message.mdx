...
function BasicFormField() {
  ...
  return (
    <FormControlProvider>
      <Label htmlFor={inputProps.input.id}>Email</Label>
...
function ErrorEmailField() {
  const fieldId = 'email'
  const [email, setEmail] = useState('')
  // This is normally handled by a third-party library
  ...
  return (
    <FormControlProvider required={true} invalid={invalid}>
      <Label htmlFor={inputProps.input.id}>Email</Label>
      <div {...inputProps.inputWrapper}>
        <input {...inputProps.input} onChange={handleChange} />
        <Show when={invalid} fallback={null}>
