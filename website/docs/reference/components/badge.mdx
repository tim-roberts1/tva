...
- `options`: A String value of a theme sentiment `'default' | 'info' | 'success' | 'warning' | 'danger'` which determines the styling color theme produced. An optional usage property that accepts a String value of `'filled' | 'outline'` to determine the styling. An optional size property that accepts a String value of either `'xs' | 's'` to display the size styles.

### Returns

An Object matching the [Anatomy](#anatomy) containing `className` and a11y properties relevant to each element/section. Additionally, it returns a `iconOptions` Object to use as the parameters for the Icon API and an `iconWrapper` property to spread on the wrapper element containing the Icon.
