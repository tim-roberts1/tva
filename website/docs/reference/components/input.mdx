...
An Object matching the [Anatomy](#anatomy) containing `className` and a11y properties relevant to the control element. Additionally, it returns a `iconOptions` property that can be passed into the Icon API, a `iconWrapper` and `invalidIconWrapper` to be placed on the container elements of both the leading and invalid state Icon.
