...
The `DangerButton` comes with three variants: text (default), medium, and strong.
...
The `DangerButton` prop-getter takes in [DangerButtonOptions](#dangerbuttonoptions) returns an Object that contains all the a11y attributes needed along with all the styles for you to use in any fashion you like - or easily extend/overwrite when needed.

```js title="Return value example"
{
  className: `ps-danger-btn ${...internal-style-modules}`
  type: 'button'
}
```
...
```typescript
getJSDangerButtonProps(options: DangerOptions): JSButtonProps
```

The CSS-in-JS API for Danger buttons (same as `getJSButtonProps`).
