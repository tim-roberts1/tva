...
<!-- TODO: ADD EXAMPLES TO REPLACE THIS CODE -->
...
For icons acting as buttons, use the `getIconButtonProps` with your desired Icon.

<!-- TODO: REPLACE CODE WITH LIVE EXAMPLES -->

```jsx title=components/EditIconButton.jsx
import { getIconProps, getIconButtonProps } from '@pluralsight/headless-styles'
import { EditIcon } from '@pluralsight/icons'

const editIconBtnProps = getIconButtonProps('edit profile')

function EditIconButton(props) {
  return (
    <button {...props} {...editIconBtnProps}>
      <EditIcon {...getIconProps()} />
    </button>
  )
}
```
...
```typescript
getIconButtonProps(): IconButtonProps
```

The icon button prop-getter doesn't take in any paramters and returns [IconButtonProps](#iconbuttonprops) that contains the a11y attributes needed for buttons with icons.
