...
When using Error messages for [Input](./Input) fields, it is recommended to pass the `ErrorMessageOptions.id` to the `InputOptions.errorId` value in order to allow your error message to be fully accessble to all users.

:::

<ErrorFormControl />

```jsx title="Make your error messages accessible by passing the same value to the InputOptions.errorId"
import { useState } from 'react'
import {
  getFormControlProps,
  getFormLabelProps,
