...
The `design-tokens` package comes with two themes: `light` and `dark` (default). Themes are controlled by using CSS classes or a `data-theme` attribute on the `html` tag. The default token values that you will receive will be for the `dark` theme. If you want to use the `light` theme, simply add a class name of `light` to the root component or `data-theme="light"` to the `html` tag.
