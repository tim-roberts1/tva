...
The `getModalProps` is used to display content separated from the current page.
Only the content within the modal can be interacted with while it is active.

:::tip

When you need to collect mandatory user confirmation, use the [Confirm Dialog](./ConfirmDialog.mdx)
