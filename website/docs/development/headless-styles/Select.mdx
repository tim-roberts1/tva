...
A placeholder option is a special option that cannot be selected by the user.
This is different from a _blank_ option, which presents a valid choice.
The placeholder should only be used when additional labelling is desired _and_ a valid, non-empty selection is required.
...
The placeholder option is not a replacement for a label.
A label is _always_ required for every form element.
