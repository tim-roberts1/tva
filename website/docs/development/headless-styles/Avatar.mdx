...
The `getAvatarProps` helper has two kind's to change the visual color of the avatar fallback that directly correlate to our [design-tokens](../design-tokens/intro#naming-convention). You can use `default` and `action`.
