...
```jsx
const { avatar, image } = getAvatarProps({ ariaLabel:  name })

<span {...avatar}>
  <img {...image} src={src} alt={initials} />
</span>
```
...
```jsx
const { avatar } = getAvatarProps({ ariaLabel:  name })
...
```jsx
const { avatar, iconOptions } = getAvatarProps({ ariaLabel:  name })
