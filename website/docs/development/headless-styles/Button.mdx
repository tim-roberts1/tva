...
`getButtonProps` comes with 3 sentiment options that correlate to our [design-tokens](../tokens/intro#naming-convention) to change the visual style of a button. You can use `action` (default), `default`, and `danger`.
...
`getButtonProps` comes with 2 usage options that correlate to our [design-tokens](../tokens/intro#naming-convention) to change the visual style of a button. You can use `outline`, and `text`.
...
You can add icons to the start or end of a button using the `icon` option and [getIconProps](./Icon.mdx) helpers with an Icon from the [Icons](../icons/intro) library.
