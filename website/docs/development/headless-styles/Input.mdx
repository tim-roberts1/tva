...
Contrary to the rest of the HTML element API's, the `date` value for the `type` attribute on an input is **not accessible**. This is a known issue in the community and unfortunately forces fully accessible applications to use the `text` value instead.
