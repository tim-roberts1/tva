...
When the user needs to move back and forth between sets of content, use two buttons that replace the displayed content with the next set from the indicated direction. If there is no more content in that direction, the button should be disabled.

This pattern is helpful when browsing ordered content, especially if when the user has previously viewed (or otherwise has less demand for) the older content, such as comments or episodic content.
