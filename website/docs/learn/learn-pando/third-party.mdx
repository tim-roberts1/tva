...
If you would like to have access to all styling and Accessibility for compatibility with Server Components, you can just the JS API.

```typescript showLineNumbers title="Return value for any JS API"
interface JSAPIReturnProps {
...
Here is an example where we are using the TextLink JS API with a Next 13 Server Component:

```jsx showLineNumbers title="A TextLink Server Component."
import {
  getJSTextLinkProps,
  getJSIconprops,
